...
def load_yolov8_model():
    return YOLO("yolov8n.pt")  # YOLOv8 Nano 모델 사용
...
def run_inference(model_path, input_path, output_dir):
    ...
    with open(txt_path, "w") as f:
        for result in results:
            if not result.boxes:  # 검출된 객체가 없는 경우
                f.write("No objects detected.\n")
                continue

            for box in result.boxes:
                cls = int(box.cls.cpu().numpy()[0])  # 클래스 ID
                conf = float(box.conf.cpu().numpy()[0])  # 신뢰도
                bbox = box.xyxy.cpu().numpy()[0]  # 바운딩 박스 좌표
                f.write(f"Class: {cls}, Confidence: {conf:.2f}, BBox: [{bbox[0]:.2f}, {bbox[1]:.2f}, {bbox[2]:.2f}, {bbox[3]:.2f}]\n")

    print(f"추론 결과가 저장되었습니다: {output_dir}")
    print(f"추론 결과 텍스트 파일 저장 완료: {txt_path}")
...
def run_webcam_detection():
    ...
    print("캡처하려면 's' 키를 누르세요. 훈련하려면 't' 키를 누르세요. 추론하려면 'i' 키를 누르세요. 종료하려면 'q' 키를 누르세요.")
    ...
    while True:
        ...
        # YOLOv8로 탐지 수행 (verbose=False로 터미널 출력 억제)
        ...
        if key == ord('s'):  # 's' 키를 눌렀을 때 데이터 저장
            ...
            run_inference("D:/ab/abc/images/test/yolov8_trained/weights/best.pt", input_path, output_dir)
